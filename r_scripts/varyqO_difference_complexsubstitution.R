# Script for producing Figure S6
...
ml_data_diff_sc_lsd <- inner_join(data_average_ml_sc, data_average_ml_lsd,
                                  ...
                                  suffix = c("_sc", "_lsd")) %>%
  mutate(diff_avg_norm_qs = avg_norm_qs_lsd - avg_norm_qs_sc)
ml_complex_data_diff_sc_all <- inner_join(data_average_ml_complex_sc, data_average_ml_complex_all,
                                  by = c("outgroup", "qratio"),
                                  suffix = c("_sc", "_all")) %>%
  mutate(diff_avg_norm_qs = avg_norm_qs_all - avg_norm_qs_sc)
ml_complex_data_diff_sc_lsd <- inner_join(data_average_ml_complex_sc, data_average_ml_complex_lsd,
                                  by = c("outgroup", "qratio"),
                                  suffix = c("_sc", "_lsd")) %>%
  mutate(diff_avg_norm_qs = avg_norm_qs_lsd - avg_norm_qs_sc)
...
figure_ml_complex_diff_sc_all <- ggplot(ml_complex_data_diff_sc_all, aes(x=outgroup, y=qratio, fill=diff_avg_norm_qs)) +
  geom_tile() + 
  labs(x="O", y="q", fill="Q1(All)-Q1(SC)") +
  theme_bw() + 
  scale_fill_gradient2(
...
figure_ml_complex_diff_sc_lsd <- ggplot(ml_complex_data_diff_sc_lsd, aes(x=outgroup, y=qratio, fill=diff_avg_norm_qs)) +
  geom_tile() + 
  labs(x="O", y="q", fill="Q1(LSD)-Q1(SC)") +
  theme_bw() + 
  scale_fill_gradient2(
...
combined_plot <- ((figure_mp_diff_sc_all | figure_mp_diff_sc_lsd) / (figure_ml_diff_sc_all | figure_ml_diff_sc_lsd) / (figure_ml_complex_diff_sc_all | figure_ml_complex_diff_sc_lsd)) + 
  plot_annotation(tag_levels = 'A')

pdf('./figures_revision/varyqO_difference_complexML.pdf', height=12, width=12)
combined_plot
dev.off()
