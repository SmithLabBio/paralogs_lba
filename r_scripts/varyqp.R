...
# read files
results_ml <- read.csv('./results/varyqp/results_ml.csv', sep=",")

results_mp <- read.csv('./results/varyqp/results_mp.csv', sep=",")
...
data_average <- results_plot %>%
  ...
  group_by(p, qratio, method, category) %>%
...
# make a box plot
figure <- ggplot(data_average, aes(x=p, y=qratio, fill=avg_norm_qs)) +
  geom_tile() + 
  labs(x="p", y="q", fill="Q1") +
...
pdf('./figures/varyqp.pdf', height=6, width=10)
figure
dev.off()

png('./figures/varyqp.png',height=6, width=10, units = "in", res=400)
figure
dev.off()
