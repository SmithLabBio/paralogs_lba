...
NOTE: EDITED TO HANDLE GAMMA-RATE AND INVARIANT SITES.
NOTE: EDITED TO COMPARE TRIMMED VS UNTRIMMED LSDs"""
...
class ConfigParser:
    ...
    def parse_config(self):
        ...
        try:
            config_dict["gamma"] = config["SeqGen"]["gamma"].split(",")
            config_dict["gamma"] = [x.strip('(') for x in config_dict["gamma"]]
            config_dict["gamma"] = [x.strip(')') for x in config_dict["gamma"]]
            config_dict["inv"] = config["SeqGen"]["inv"]
        except KeyError as e:
            config_dict["gamma"] = None
            config_dict["inv"] = None
        except Exception as e:
            config_dict["gamma"] = None
            config_dict["inv"] = None
...
class DataSimulator:
    ...
    def simulate_seqgen(self):
        ...
        for i in range(self.config_dict["replicates"] * self.config_dict["subreps"]):
            ...
            if self.config_dict["gamma"] is None and self.config_dict["inv"] is None:
                # create command
            ...
            elif self.config_dict["gamma"] is None:
                # create command
                command = "%s -l %s -m %s -i %s temp.tre > %s_%s.phy" % (
                    self.config_dict["seqgen"],
                    self.config_dict["length"],
                    self.config_dict["model"],
                    self.config_dict["inv"],
                    self.config_dict["output prefix"],
                    str(i)
                )
            elif self.config_dict["inv"] is None:
                # create command
                command = "%s -l %s -m %s -a %s -g %s temp.tre > %s_%s.phy" % (
                    self.config_dict["seqgen"],
                    self.config_dict["length"],
                    self.config_dict["model"],
                    self.config_dict["gamma"][0],
                    self.config_dict["gamma"][1],
                    self.config_dict["output prefix"],
                    str(i)
                )
            else:
                # create command
                command = "%s -l %s -m %s -i %s -a %s -g %s temp.tre > %s_%s.phy" % (
                    self.config_dict["seqgen"],
                    self.config_dict["length"],
                    self.config_dict["model"],
                    self.config_dict["inv"],
...
class ResultsWriter:
    ...
    def write_results_lsdfocus(self, results_q1_trimmed, results_q2_trimmed, results_q3_trimmed, results_q1_untrimmed, results_q2_untrimmed, results_q3_untrimmed, name, params):

        filename = os.path.join(self.config_dict["output directory"], self.config_dict["output prefix"], name+'.csv')

        results_index = 0
        with open(filename, 'w') as f:
            f.write('prefix,paramfile,duplication rate,loss rate,p,qratio,r,outgroup,q1_trimmed,q2_trimmed,q3_trimmed,q1_untrimmed,q2_untrimmed,q3_untrimmed,replicate\n')
            for i in range(self.config_dict["replicates"]):
                for j in range(self.config_dict["subreps"]):
                    f.write(f"{self.config_dict['output prefix']},{params},{self.config_dict['duplication rate'][i]},{self.config_dict['loss rate'][i]},"
                                f"{self.config_dict['p'][i]},{self.config_dict['qratio'][i]},{self.config_dict['r'][i]},{self.config_dict['outgroup']},"
                                f"{results_q1_trimmed[results_index]},{results_q2_trimmed[results_index]},{results_q3_trimmed[results_index]},"
                                f"{results_q1_untrimmed[results_index]},{results_q2_untrimmed[results_index]},{results_q3_untrimmed[results_index]},"
                                f"{results_index}\n")

                    results_index += 1


class LSDTreeInferrer:

    def __init__(self, config_dict):
        self.config_dict = config_dict
    
    def lsd_analysis(self, trees_to_check):
        ...
        for i in range(self.config_dict["replicates"] * self.config_dict["subreps"]):
            if "None" in all_trees[i]:
                ...
                for node in t.traverse("postorder"):
                    ...
                    if len(species) == 1 and len(samples) > 1 and samples[0] == "B":
                        indicator = True
                        lsd_samples = lsd_samples + node.get_leaf_names()
                        lsd_species.append(samples[0])
                        lsd_count+=1
                    elif len(species) == 1 and len(samples) > 1 and samples[0] == "D":
                        indicator = True
                        lsd_samples = lsd_samples + node.get_leaf_names()
            ...
            if indicator and indicator_only:
                lsd = True
            else:
                lsd = False
            
            # get the alignment if lsd indicator is true
            if lsd:

                lsd_samples = list(set(lsd_samples))
                lsd_species = list(set(lsd_species))

                # find samples to remove
                to_remove = []
                for spec in lsd_species:
                    spec_samples = [x for x in lsd_samples if x.split("_")[0] == spec]
                    spec_samples.sort()
                    to_remove.append(spec_samples[1:])
                to_remove_flat = [item for sublist in to_remove for item in sublist]

                # trim alignment
                newalignmentfile = f"{self.config_dict['output prefix']}_{i}_trimmed.phy"
                start=True
                with open(f"{self.config_dict['output prefix']}_{i}.phy", 'r') as file:
                    for line in file.readlines():
                        if start:
                            seqcount = int(line.split()[0])
                            length = int(line.split()[1])
                            newseqcount = seqcount - len(to_remove_flat)
                            start = False
                            if newseqcount >=4:
                                newalignment = open(newalignmentfile, 'w')
                                newalignment.write(f" {newseqcount} {length}\n")
                        elif newseqcount >=4:
                            if line.split()[0] not in to_remove_flat:
                                newalignment.write(line)
                if newseqcount >=4:
                    newalignment.close()
                
                    # run mp inference on untrimmed alignment
                    command = '%s -s %s_%s.phy -pre rep%s_untrimmed' % (
                        self.config_dict["mpboot"],
                        self.config_dict["output prefix"],
                        str(i),
                        str(i)
                    ...
                    # run mp inference on trimmed alignment
                    command = '%s -s %s_%s_trimmed.phy -pre rep%s_trimmed' % (
                        self.config_dict["mpboot"],
                        self.config_dict["output prefix"],
                        str(i),
                        str(i)
                    ...
                    # trim untrimmed trees
                    with open(f'rep{i}_untrimmed.treefile', 'r') as f:
                        untrimmed_tree = f.read()
                        untrimmedt = ete3.Tree(untrimmed_tree)
                        tokeep = [x for x in untrimmedt.get_leaf_names() if x not in to_remove_flat]
                        untrimmedt.prune(tokeep, preserve_branch_length=True)
                        untrimmedt.write(format=1, outfile=f'rep{i}_untrimmed_trimmed.treefile')
                    
                    # run ASTRAL
                    num_leaves = 0
                    spdict = {}
                    for leaf in t.get_leaf_names():
                        spname = leaf.split('_')[0]
                        # check if key exists
                        if spname in spdict:
                            spdict[spname] = spdict[spname] + [leaf]
                        else:
                            spdict[spname] = [leaf]
                    ...
                    if len(spdict) < 4:
                        ...
                        # create command
                        command1 = f"{self.config_dict['astral']} -C -c {self.config_dict['score tree']} -i rep{i}_untrimmed_trimmed.treefile -a temp.map -u 2 -r 1 -s 0 > temp_untrimmed_trimmed.scored.log"
                        command2 = f"{self.config_dict['astral']} -C -c {self.config_dict['score tree']} -i rep{i}_trimmed.treefile -a temp.map -u 2 -r 1 -s 0 > temp_trimmed.scored.log"
                        os.system(command1)
                        os.system(command2)

                        scored_tree_untrimmed_trimmed = dendropy.Tree.get(path="temp_untrimmed_trimmed.scored.log", schema="newick")
                        scored_tree_trimmed = dendropy.Tree.get(path="temp_trimmed.scored.log", schema="newick")
                        
                        filter_fn = lambda n: hasattr(n, 'label') and n.label is not None 
                        nodes = scored_tree_untrimmed_trimmed.find_node(filter_fn=filter_fn)
                        children = [x.taxon.label for x in nodes.child_nodes()]
                        if ('C' in children and 'D' in children) or ('A' in children and 'B' in children):
                            q1_untrimmed = nodes.label.split("q1=")[1].split(";")[0]
                            q2_untrimmed = nodes.label.split("q2=")[1].split(";")[0]
                            q3_untrimmed = nodes.label.split("q3=")[1].split("]")[0]
                        else:
                            raise Exception("Issue when scoring tree.")
                        nodes = scored_tree_trimmed.find_node(filter_fn=filter_fn)
                        children = [x.taxon.label for x in nodes.child_nodes()]
                        if ('C' in children and 'D' in children) or ('A' in children and 'B' in children):
                            q1_trimmed = nodes.label.split("q1=")[1].split(";")[0]
                            q2_trimmed = nodes.label.split("q2=")[1].split(";")[0]
                            q3_trimmed = nodes.label.split("q3=")[1].split("]")[0]
                        else:
                            raise Exception("Issue when scoring tree.")
                    os.system('rm temp.map temp_untrimmed_trimmed.scored.log temp_trimmed.scored.log')
                else:
                    q1_trimmed = "NA"
                    q2_trimmed = "NA"
                    q3_trimmed = "NA"
            ...
            q1_trimmed_list.append(q1_trimmed)
            q2_trimmed_list.append(q2_trimmed)
            q3_trimmed_list.append(q3_trimmed)
            q1_untrimmed_list.append(q1_untrimmed)
            q2_untrimmed_list.append(q2_untrimmed)
            q3_untrimmed_list.append(q3_untrimmed)
            del q1_trimmed, q2_trimmed, q3_trimmed, q1_untrimmed, q2_untrimmed, q3_untrimmed

            del(indicator, indicator_only)




        os.system('find . -maxdepth 1 -name "rep*.treefile" -delete')
        os.system('find . -maxdepth 1 -name "rep*.mpboot" -delete')
        os.system('find . -maxdepth 1 -name "rep*.log" -delete')
        os.system('find . -maxdepth 1 -name "rep*.parstree" -delete')


        # change directory
        os.chdir(startdir)
        return(q1_trimmed_list, q2_trimmed_list, q3_trimmed_list, q1_untrimmed_list, q2_untrimmed_list, q3_untrimmed_list)
